...
fn info(word: &impl Authentication){
    p("The user information is {}", word.auth());
}
pub struct User {
        name:String,
        age: u64,
...
impl Authentication for User{
    fn auth(&self) -> String{
        println!{"The man's name is {}, he is {} years of age and has a favorite color of {}", self.name,self.age, self.fav_color};
    }
}
impl Authentication for Car {
    fn auth(&self)->String{
        println!{"The brand of this car is {}, {} engine, and it 's color{} ", self.name, self.drive, self.color};
    }
}
fn user_car(){
