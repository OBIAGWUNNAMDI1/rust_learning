...
#[derive(Debug)] //  A macro that helps in debugging and printing for structs, and enums.__rust_force_expr!
fn using_Result(number: f64 , divisor:f64) -> Result<f64, error>{
    if divisor == 0.0{
        Err(error)    
    }else {
        Ok(number /divisor)
...
fn main(){
    ...
    panic_backtrace();
    using_Result(50.0 ,4.0);
    using_Result(20.0 , 0.0)
    colour_checker();
