...
//The result Type has a lot og methods which helps to handle and propagate errors. Result.expect, Result.unwrap, Result.Ok, Result.err, Result.unwrap_or().
#[derive(Debug)] //  A macro that helps in debugging and printing for structs, and enums.
struct Message;
fn using_Result(number: f64 , divisor:f64) -> Result<f64, Message>{
    if divisor == 0.0{
        Err(Message)    
    }else {
        Ok(number /divisor)
...
fn main(){
    ...
    panic_backtrace();
    println!("{:?}", using_Result(50.0 ,4.0)); //Output  Ok(12.5) 
    println!("{:?}", using_Result(20.0 , 0.0)) //Output  Err(Message)
    colour_checker();
